...
export default function BeatCard({ beat, fromCents, delay }: { beat: ClientBeat; fromCents: number; delay: number }) {
  ...
  const cover = beat.coverUrl
    ...
    : `radial-gradient(circle at 30% 25%, rgba(157,107,255,.5), transparent 55%), linear-gradient(150deg, var(--v-deep), var(--bg-0))`;

  return (
    <div className="card reveal" ref={ref} onMouseMove={onMove} onMouseLeave={onLeave} style={{ transitionDelay: `${delay}s` }}>
      <Link href={`/product/${beat.slug}`} style={{ textDecoration: "none", color: "inherit", display: "block" }}>
        <div className="card-art">
          ...
          {!sold && beat.previewUrl &&
            <div className="play" onClick={(e) => { e.preventDefault(); e.stopPropagation(); play({ id: beat.id, title: beat.title, meta: `${beat.bpm ?? ""} BPM · ${beat.music_key ?? ""} · ${beat.genre ?? ""}`, coverUrl: beat.coverUrl, previewUrl: beat.previewUrl }); }}>
